...
switch ($_POST["payment_status"]) {
     case 'failure':
            echo "<h1>Su pago falló o fue rechazado</h1>";
         break;

    case 'pending':
    case 'in_process':
        echo "<h1>Su pago se encuentra pendiente.</h1>";
         break;

     case 'approved':
        if(isset($_POST["merchant_order_id"])){
            $payment = MercadoPago\MerchantOrder::find_by_id($_POST["merchant_order_id"]);
        }
        echo "<h1>¡Su pago fue exitoso!</h1>";
        echo "<h2>Monto: ".$payment->paid_amount."</h2>";
        echo "<h2>Numero de orden del pedido: ".$_POST["external_reference"]."</h2>";
        echo "<h2>ID Pago de MercadoPago: ".$_POST["payment_id"]."</h2>";
         break;
     
     default:   
            echo "<h1>No se ha abonado el pago. Intente nuevamente.</h1>";
         break;
 } 
